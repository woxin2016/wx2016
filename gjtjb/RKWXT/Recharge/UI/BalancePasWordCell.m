...
@interface BalancePasWordCell ()
{
    WXUITextField *_textField;
}
@end

@implementation BalancePasWordCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [_textField setPlaceholder:@"请输入充值卡密码"];
...
- (void)textValueDidChanged:(id)sender{
    if(_delegate && [_delegate respondsToSelector:@selector(passWordTextFiledPhoneValueDidChanged:)]){
        [_delegate passWordTextFiledPhoneValueDidChanged:_textField.text];
    }
}
