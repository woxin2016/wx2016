...
@implementation VirtualUserInfoCell

+ (instancetype)VirtualUserInfoCellWithTabelView:(UITableView*)tableView{
    NSString *identifier = @"VirtualUserInfoCell";
    VirtualUserInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[VirtualUserInfoCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    return cell;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        downImgView.frame = CGRectMake(0, [VirtualUserInfoCell cellHeightOfInfo:nil]-2*yOffset-2, IPHONE_SCREEN_WIDTH, 2);
